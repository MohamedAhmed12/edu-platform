...
(function ($) {
  $.fn.fieldsLinker = function (action, input, selector) {
    var factory = this;
    ...
    if (action == "init") {
      ...
      console.log(
        factory.work.errMsg + "no action parameter provided (param 1)"
      );
    }
  };
})(jQuery);
...
function FieldsLinker(selector) {
  ...
  this.draw = function () {
    ...
    links.forEach(function (item, i) {
      ...
      if (
        self.lineStyle == "square-ends" ||
        self.lineStyle == "square-ends-dotted"
      ) {
    ...
    if (!already) {
      if (self.associationMode == "oneToOne") {
        for (var i = self.linksByName.length - 1; i >= 0; i--) {
          if (
            self.linksByName[i].tables == tablesAB &&
            self.linksByName[i].to == infos.nameB
          ) {
        ...
        for (var i = self.linksByName.length - 1; i >= 0; i--) {
          if (
            self.linksByName[i].tables == tablesAB &&
            self.linksByName[i].from == infos.nameA
          ) {
            self.linksByName.splice(i, 1);
          }
        }
  ...
  this.readUserPreferences = function () {
    ...
    if (self.data.options.lineStyle) {
      if (
        self.data.options.lineStyle == "square-ends" ||
        self.data.options.lineStyle == "square-ends-dotted"
      )
  ...
  this.makeDropDownForLists = function () {
    ...
    self.dropDownForLists.css("width", "100%");
    self.listNames.forEach(function (x) {
      var $option = $("<option></option>");
      $option.val(x).text(x).appendTo(self.dropDownForLists);
    });
  };
  this.drawColumnsAtLoadTime = function () {
    ...
    self.$ulLeft = $("<ul></ul>");
    self.$ulLeft.appendTo(self.$leftDiv).css({
      "text-align": "left",
      "list-style": "none",
    });

    self.$midDiv = $("<div></div>");
    self.$midDiv.appendTo(self.$main).addClass("FL-mid").css({
      display: "inline-block",
      width: "20%",
    });
    ...
    self.$ulRight = $("<ul></ul>");
    self.$ulRight.appendTo(self.$leftDiv).css({
      "text-align": "left",
      "list-style": "none",
    });
  };
  ...
  this.drawColumnsContentA = function () {
    ...
    self.$ulLeft.attr("data-col", self.chosenListA).css({
      "text-align": "left",
      "list-style": "none",
    });
  ...
  this.drawColumnsContentB = function () {
    ...
    self.listB.forEach(function (x, i) {
      ...
      if (isMandatory && self.mandatoryTooltips) {
        ...
        if (self.bootstrap_enabled) $li.tooltip();
      }
    });
    // Computing the vertical offset of the middle of each cell.
    self.ListHeights2 = [];
    ...
    $(self.$leftDiv)
      ...
      .on("change", function (e) {
        ...
        $(self.$rightDiv)
          .find("select option")
          .each(function () {
            $(this).attr("disabled", $(this).val() == self.chosenListA);
          });
    ...
    $(self.$rightDiv)
      ...
      .on("change", function (e) {
        ...
        $(self.$leftDiv)
          .find("select option")
          .each(function () {
            $(this).attr("disabled", $(this).val() == this.chosenListB);
          });
    ...
    if (!self.onError) {
      if (input) {
        ...
        if (options.associationMode) {
          ...
          if (
            self.associationMode == "oneToOne" &&
            formerAssociation == "manyToMany"
          ) {
            let unicityDict = {};
            for (var i = self.linksByName.length - 1; i >= 0; i--) {
              unicityTokenA =
                self.linksByName[i].tables +
                "_A_" +
                self.linksByName[i]["from"];
              unicityTokenB =
                self.linksByName[i].tables + "_B_" + self.linksByName[i]["to"];
              let doDelete = false;
  ...
  this.enable = function (doEnable) {
    ...
    $(self.$root).find(".eraseLink").prop("disabled", self.isDisabled);

    if (doEnable) {
      $(self.$root).find("li").removeClass("inactive");
    } else {
      $(self.$root).find("li").addClass("inactive");
    }

    $(self.$root).find("select").prop("disabled", self.isDisabled);
