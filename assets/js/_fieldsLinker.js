...
(function ($) {
  ...
  var onError = false;
  var className = "fieldsLinker col-12 col-sm-12";
  var linksByName = [];
  var ListHeights1 = [];
  var ListHeights2 = [];
  var move = null;
  ...
  var handleColor = "#4994FE, #4994FE".split(",");
  var lineColor = "black";
  ...
  var draw = function () {
    window.linksByName = linksByName;
    ...
    links.forEach(function (item, i) {
      ...
      if (lineStyle == "square-ends" || lineStyle == "square-ends-dotted") {
        ...
        canvasCtx.arc(Bx - 8, By, 8, 0, 2 * Math.PI, false);
        canvasCtx.strokeStyle = "#4994FE";
        canvasCtx.fillStyle = "#4994FE";
        ...
        canvasCtx.moveTo(Bx - 15, By - 5);

        canvasCtx.lineTo(Bx + 8, By + 8);
        canvasCtx.lineTo(Bx + 4, By + 4);

        canvasCtx.strokeStyle = "#fff";
        // canvasCtx.moveTo( Bx-30,  By-30);

        // canvasCtx.lineTo(Bx+10, By+20);
        // canvasCtx.lineTo(Bx+9, By+19);
        canvasCtx.stroke();
      } else {
        canvasCtx.strokeStyle = "4994FE";
        canvasCtx.lineTo(Bx, By);
  ...
  var makeLink = function (infos) {
    ...
    var test = linksByName.filter(function (x) {
      return (
        x.tables == tablesAB && x.to == infos.nameB && x.from == infos.nameA
      );
    });
    ...
    if (!already) {
      if (associationMode == "oneToOne") {
        for (var i = linksByName.length - 1; i >= 0; i--) {
          if (
            linksByName[i].tables == tablesAB &&
            linksByName[i].to == infos.nameB
          ) {
        ...
        for (var i = linksByName.length - 1; i >= 0; i--) {
          if (
            linksByName[i].tables == tablesAB &&
            linksByName[i].from == infos.nameA
          ) {
            linksByName.splice(i, 1);
          }
        }
  ...
  var readUserPreferences = function () {
    ...
    if (data.options.lineStyle) {
      if (
        data.options.lineStyle == "square-ends" ||
        data.options.lineStyle == "square-ends-dotted"
      )
  ...
  var makeDropDownForLists = function () {
    dropDownForLists = $("<h3></h3>");
    dropDownForLists.css("width", "100%");
    listNames.forEach(function (x) {
      var $option = $("<span></span>");
      $option.val(x).text(x).appendTo(dropDownForLists);
    });
  };
  var drawColumnsAtLoadTime = function () {
    ...
    $midDiv = $("<div></div>");
    $midDiv.appendTo($main).addClass("FL-mid").css({
      display: "inline-block",
      width: "20%",
    });
    $rightDiv = $("<div></div>");
  ...
  var createFilterDiv = function (listIndex) {
    ...
    let searchBtn = $(
      '<button role="button" type="button" class="btn btn-small">' +
        data.options.buttonFilter +
        "</button>"
    );
    ...
    searchBtn.click(function (e) {
      ...
      FL_Factory_Lists.Lists[listIndex] = JSON.parse(
        JSON.stringify(FL_Original_Factory_Lists.Lists[listIndex])
      );
      var filter = $(e.target).parent().find("input").val();
      FL_Factory_Lists.Lists[listIndex].list = filterList(
        FL_Factory_Lists.Lists[listIndex].list,
        filter,
        FL_Factory_Lists.Lists[listIndex].keysFilter
      );
  ...
  var filterList = function (list, filter, keysFilter) {
    ...
    list.forEach((item) => {
      let found = false;
      for (let i = 0; i < keysFilter.length && !found; i++) {
  ...
  var drawColumnsContentA = function () {
    ...
    $ulA.appendTo($leftDiv).attr("data-col", chosenListA).css({
      "text-align": "left",
      "list-style": "none",
    });
    ...
    listA.forEach(function (x, i) {
      ...
      $li.appendTo($ulA).attr("data-offset", i).attr("data-name", id).css({
        width: "100%",
        position: "relative",
      });
      var $div = $("<div></div>");
      ...
      $pullIcon
        .appendTo($li)
        .addClass("fa   fa-location-arrow  link")
        .attr("draggable", "false")
        .css({
          // 'right': '8px',
          color: "#aaa",
  ...
  var drawColumnsContentB = function () {
    ...
    $ulB.appendTo($rightDiv).attr("data-col", chosenListB).css({
      "text-align": "left",
      "list-style": "none",
    });
    ...
    listB.forEach(function (x, i) {
      ...
      if (isMandatory && mandatoryTooltips) {
        $li.attr("data-placement", "top").attr("title", mandatoryErrorMessage);
        if (bootstrap_enabled) $li.tooltip();
      }
    });
    // Computing the vertical offset of the middle of each cell.
    ListHeights2 = [];
    ...
    if (action == "init") {
      ...
      if (displayMode == "original") {
        ...
        if (!data.Lists[0].keyName || !data.Lists[1].keyName) {
          onError = true;
          console.log(
            errMsg + "alternateView mode : provide keyName properties"
          );
          return;
        }
      }
        ...
        if (input) {
          ...
          if (options.associationMode) {
            ...
            if (
              ...
              formerAssociation == "manyToMany"
            ) {
              let unicityDict = {};
              for (var i = linksByName.length - 1; i >= 0; i--) {
                unicityTokenA =
                  linksByName[i].tables + "_A_" + linksByName[i]["from"];
                unicityTokenB =
                  linksByName[i].tables + "_B_" + linksByName[i]["to"];
                let doDelete = false;
      ...
      isDisabled = true;
      $(factory).find(".eraseLink").prop("disabled", isDisabled);
      $(factory).find("li").addClass("inactive");
      $(factory).find("select").prop("disabled", isDisabled);
      globalAlpha = 0.5;
      draw();
      return that;
      ...
      isDisabled = false;
      $(factory).find(".eraseLink").prop("disabled", isDisabled);
      $(factory).find("li").removeClass("inactive");
      $(factory).find("select").prop("disabled", isDisabled);
      globalAlpha = 1;
      draw();
      return factory;
