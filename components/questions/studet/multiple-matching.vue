  ...
  methods: {
    setAnswer(e) {
      ...
      let answers = this.answers
        .map((link, index) => {
          let qFiltered = this.questionData.questions.find((q) => {
            q = q.media.url
              ? `<img class="img-fluid mb-3" src="${q.media.url}">` + q.text
              : q.text;

            return link.from === q;
          });
          let q = qFiltered || {};
          let opFiltered = this.questionData.options.find(
    ...
    loadCorrectAnswers(questionData) {
      ...
      questionData.options.map((option) => {
        questionData.questions
          .filter((q) => {
            return option.questions.includes(q.id);
          })
          .map((q) => {
            existingLinks.push({
              from:
