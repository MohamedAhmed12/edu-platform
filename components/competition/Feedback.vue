<template>
  ...
  >
    ...
    <h1 class="text-center">
      <span v-if="this.$store.state.i18n.locale == 'ar'">{{
        $t("feedback")
      }}</span>
      <span>{{ feedback.name }}</span>
      <span v-if="this.$store.state.i18n.locale == 'en'">{{
        $t("feedback")
      }}</span>
      <span>{{ $t("on_competition") }}</span>
    </h1>
    <div
...
export default {
  ...
  methods: {
    load() {
      this.$axios
        .get(
          `student/competitions/${this.$route.params.id}/student/${this.$route.params.student}`
        )
        .then((response) => {
