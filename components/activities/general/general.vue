<template>
  <div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 mt-30">
      <div class="row m-15 blue">
        <div
          class="col-md-6 col-sm-12 co-12 m-auto mobile-padding"
          :class="{
            'col-md-12':
              this.$auth.user.attributes['user-type'] == 'student_teacher',
          }"
        >
          <p class="d-inline-block">
            <i aria-hidden="true" class="fa fa-bell" />
            {{ $t("genaral_activity") }}
          </p>
        </div>
        <div
          v-if="this.$auth.user.attributes['user-type'] == 'parent'"
          class="col-md-6 col-sm-12 co-12 m-auto mobile-padding text-right"
        >
          <div v-if="$i18n.locale === 'en'">
            <DatePicker
              v-model="form"
              format="yyyy/MM/dd"
              :placeholder="$t('date_from')"
              style="width: 200px;"
              type="date"
            />
            <DatePicker
              v-model="to"
              format="yyyy/MM/dd"
              :placeholder="$t('date_to')"
              style="width: 200px;"
              type="date"
            />
            <button
              class="btn-cancel px-4 py-1 ml-3 text-white"
              @click="get(page, data_from, data_to), filter()"
            >
              {{ $t("Filter") }}
            </button>
          </div>
          <div v-if="$i18n.locale === 'ar'">
            <DatePicker
              v-model="to"
              format="yyyy/MM/dd"
              :placeholder="$t('date_from')"
              style="width: 200px;"
              type="date"
            />
            <DatePicker
              v-model="form"
              format="yyyy/MM/dd"
              :placeholder="$t('date_to')"
              style="width: 200px;"
              type="date"
            />
            <button
              class="btn-cancel px-4 py-1 ml-3 text-white"
              @click="get(page, data_from, data_to), filter()"
            >
              {{ $t("Filter") }}
            </button>
          </div>
        </div>
      </div>
      <div v-if="generalData.length == 0" class="row m-auto">
        <div class="col-md-12">
          <h2 class="text-center">
            {{ $t("no Data Found") }}
          </h2>
        </div>
      </div>
      <div class="row m-15 m-t-0">
        <div id="style-3" class="col-md-12 p-l-14 tabel-hight scrollbar">
          <div
            v-for="(activity, i) in generalData"
            :key="i"
            class="row tabel-block"
          >
            <div class="col-md-10 col-sm-9 col-12">
              <p class="first-col">
                {{ activity.text }}
              </p>
              <p class="first-col">
                {{ activity.exam_title }}
              </p>
            </div>
            <div class="col-md-2 col-sm-3 col-12 mobile-margin text-center p-0">
              <p class="second-col">
                <i class="far fa-clock" /> {{ activity.date }}
              </p>
            </div>
          </div>
          <div id="tri" ref="tri" class="col-md-12 tri" />
        </div>
        <div v-show="loader" class="col-md-12 text-center m-auto">
          <svg
            height="100px"
            preserveAspectRatio="xMidYMid"
            style="
              margin: auto;
              background: none;
              display: block;
              shape-rendering: auto;
            "
            viewBox="0 0 100 100"
            width="100px"
            xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink"
          >
            <defs>
              <path
                id="path"
                d="M50 15A15 35 0 0 1 50 85A15 35 0 0 1 50 15"
                fill="none"
              />
              <path
                id="patha"
                d="M0 0A15 35 0 0 1 0 70A15 35 0 0 1 0 0"
                fill="none"
              />
            </defs>
            <g transform="rotate(0 50 50)">
              <use stroke="#dfdfdf" stroke-width="3" xlink:href="#path" />
            </g>
            <g transform="rotate(60 50 50)">
              <use stroke="#dfdfdf" stroke-width="3" xlink:href="#path" />
            </g>
            <g transform="rotate(120 50 50)">
              <use stroke="#dfdfdf" stroke-width="3" xlink:href="#path" />
            </g>
            <g transform="rotate(0 50 50)">
              <circle cx="50" cy="15" fill="#e15b64" r="9">
                <animateMotion begin="0s" dur="1s" repeatCount="indefinite">
                  <mpath xlink:href="#patha" />
                </animateMotion>
              </circle>
            </g>
            <g transform="rotate(60 50 50)">
              <circle cx="50" cy="15" fill="#f8b26a" r="9">
                <animateMotion
                  begin="-0.16666666666666666s"
                  dur="1s"
                  repeatCount="indefinite"
                >
                  <mpath xlink:href="#patha" />
                </animateMotion>
              </circle>
            </g>
            <g transform="rotate(120 50 50)">
              <circle cx="50" cy="15" fill="#abbd81" r="9">
                <animateMotion
                  begin="-0.3333333333333333s"
                  dur="1s"
                  repeatCount="indefinite"
                >
                  <mpath xlink:href="#patha" />
                </animateMotion>
              </circle>
            </g>
          </svg>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  props: ["total", "get"],
  data() {
    return {
      generalData: [],
      page: 1,
      maxPerPage: 10,
      to: "",
      form: "",
      loader: false,
    };
  },
  computed: {
    pageCount() {
      return Math.ceil(this.total / this.maxPerPage);
    },
    data_to() {
      return this.to ? this.to.toISOString().split("T")[0] : "";
    },
    data_from() {
      return this.form ? this.form.toISOString().split("T")[0] : "";
    },
    dataAct() {
      return this.$store.getters["activity-logs/logs"];
    },
  },
  watch: {
    dataAct(nwev) {
      this.loader = true;
      this.generalData.push(...nwev);
      setTimeout(() => {
        this.loader = false;
      }, 700);
    },
  },
  mounted() {
    this.scrollTigger();
  },
  methods: {
    scrollTigger() {
      const observer = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          if (entry.intersectionRatio > 0 && this.page < this.pageCount) {
            this.page++;
            this.get(this.page, this.data_from, this.data_to, true);
          }
        });
      });
      observer.observe(this.$refs.tri);
    },
    filter() {
      this.generalData = [];
      this.loader = false;
      setTimeout(() => {
        this.loader = true;
      }, 700);
    },
  },
};
</script>

<style scoped>
.tri {
  height: 50px;
}
</style>
